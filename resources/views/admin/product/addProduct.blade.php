...
<div class="p-8 bg-white rounded-lg shadow-md mx-auto">
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" onsubmit="return validateImages()">
        @csrf
        ...
        <div class="mb-6 relative">
            ...
            <div class="relative mt-1">
                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Saisissez le nom du produit"
                       class="w-full pl-10 px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300 @error('name') border-red-500 @enderror">
        ...
        <div class="mb-6 relative">
            ...
            <textarea id="description" name="description" rows="4" placeholder="Entrez une description du produit"
                      class="w-full px-4 py-2 mt-1 border rounded-lg focus:ring focus:ring-blue-300 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
            @error('description')
            <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-6 relative">
            ...
            <div class="relative mt-1">
                <input type="number" id="price" name="price" step="0.01" value="{{ old('price') }}" placeholder="0.00"
                       class="w-full pl-10 px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300 @error('price') border-red-500 @enderror">
                <span class="absolute left-3 top-3 text-gray-400">💰</span>
            </div>
            @error('price')
            <span class="text-red-500 text-sm">{{ $message }}</span>
        ...
        <div class="mb-6 relative">
            ...
            <div class="relative mt-1">
                <input type="number" id="quantity" min="0" max="500" name="quantity" value="{{ old('quantity') }}" placeholder="0"
                       class="w-full pl-10 px-4 py-2 border rounded-lg focus:ring focus:ring-blue-300 @error('quantity') border-red-500 @enderror">
                <span class="absolute left-3 top-3 text-gray-400">📦</span>
            </div>
            @error('quantity')
            <span class="text-red-500 text-sm">{{ $message }}</span>
